...
char **prune_word_list(char **words, int len, int *words_remaining) {
	int i = 0;
	char **word_list_to_return = malloc(1 * sizeof(*word_list_to_return));
	//Error checking
	...
	//Iterate through the whole list to find all the words of length len
	while (words[i] != NULL){
		if (strlen(words[i]) == len){
			*words_remaining += 1;
			//Put into array by reallocating one more pointer space
			word_list_to_return = realloc(word_list_to_return, *words_remaining * (sizeof(*word_list_to_return)));
			//Error checking
			if (word_list_to_return == NULL) {
	        		perror("realloc");
	        		exit(1);
    			}
			word_list_to_return[*words_remaining - 1] = words[i];
		}
		i += 1;
	}
	//Append null at the end of the list to mark the end of the list
	word_list_to_return = realloc(word_list_to_return, (*words_remaining + 1) * (sizeof(*words)));
	//Error checking
	if (word_list_to_return == NULL) {
	        perror("realloc");
	        exit(1);
    	}
	word_list_to_return[*words_remaining] = NULL;
	
	//Free an empty word list
	if (*words_remaining == 0){
		free(word_list_to_return);
		return NULL;
	}
...
char **get_word_list_of_length(char **words, int *len) {
	...
	char **word_list_of_length;
	char *string_in;
	char line[BUF_SIZE];
	
	//While the word list is empty
	while(words_remaining == 0){
		//Keep asking the user if they give inappropriate input
		while (length_of_word <= 0){
			...
			length_of_word = strtol(line, &string_in, 10);
		
			//If string_in[0] == '\n', the user has entered only a number
			if (length_of_word <= 0 || string_in[0] != '\n'){
				printf("There are no words of that length.\n");
				length_of_word = 0;
			}
		}
		//Store it in len
		*len = length_of_word;
		word_list_of_length = prune_word_list(words, *len, &words_remaining);
