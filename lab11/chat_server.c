...
int accept_connection(int fd, struct sockname *usernames) {
	...
	//Malloc some space for the username
	char* username = malloc(sizeof(char)*num_read+1);
	strncpy(username, buf, num_read + 1);
...
int read_from(int client_index, struct sockname *usernames) {
	...
	//Get the username and copy it over to a string that will be used to write
	char *username = usernames[client_index].username;
	char username_to_write[BUF_SIZE];
	strcpy(username_to_write, username);
	strncat(username_to_write, ": ", BUF_SIZE-strlen(username_to_write));
	strncat(username_to_write, buf, BUF_SIZE - strlen(username_to_write));
	...
	for(int i = 0;i < MAX_CONNECTIONS;i++){
		if (usernames[i].sock_fd > -1) {
			if(write(usernames[i].sock_fd, username_to_write, strlen(username_to_write)) != strlen(username_to_write)) {
				usernames[client_index].sock_fd = -1;
			}
		}
	}
...
int main(void) {
    ...
    while (1) {
        ...
        for (int index = 0; index < MAX_CONNECTIONS; index++) {
			...
			//Free the usernames and reset them when we disconnect
