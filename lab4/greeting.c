...
int main(int argc, char **argv) {
    ...
    char greeting[20];
    char *name = argv[2];
	int space_in_string = 19;

	greeting[19] = 0;
	//Only want 19 characters max from the greeting input
	strncpy(greeting, argv[1], space_in_string);
	//Subtract the space available just in case the greeting wasn't super long
	space_in_string -= strlen(greeting);
	//Only append if there is space in the greeting
	strncat(greeting, " ", space_in_string);

	if (space_in_string != 0){
		//Decrease by one for the new space
		space_in_string -= 1;
	}
	strncat(greeting, name, space_in_string);
